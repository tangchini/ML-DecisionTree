...
class multiDecisionTrees:
    ...
    def tree_generator(self):
        trees = []
        if self.method == 'mix':
            entrop_nums = int(np.random.randint(30,70)*0.01 * self.tree_num)
            method_list = ['entropy']*entrop_nums+['gini']*(self.tree_num-entrop_nums)
            for i in range(self.tree_num):
                trees.append(decisionTree(max_depth=self.max_depth, method=method_list[i]))
        else:
            for i in range(self.tree_num):
...
# single tree
clf = decisionTree(max_depth=10, method='entropy')
clf.fit(train_X, train_y)

# multiple trees
# clf_forest = multiDecisionTrees(tree_num=5, max_depth=11, method='mix')
...
# test_X_predicted_trees = clf_forest.trees_predict(test_x)
...
# err = errorRate(highest_score, test_X_predicted_trees)
# print('simulated kaggle score:', err)